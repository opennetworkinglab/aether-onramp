...
collect_pod_diagnostics() {
  ...
  while IFS= read -r pod_name; do
    ...
    if ! kubectl describe pod -n "$namespace" "$pod_name" > "$output_dir/${pod_name}_describe.log" 2>&1; then
      log_error "Warning: failed to describe pod ${pod_name}"
    fi

    if ! kubectl logs --all-containers -n "$namespace" "$pod_name" > "$output_dir/${pod_name}.log" 2>&1; then
      log_error "Warning: failed to collect all-container logs for pod ${pod_name}; attempting fallback"
      if ! kubectl logs -n "$namespace" "$pod_name" >> "$output_dir/${pod_name}.log" 2>&1; then
